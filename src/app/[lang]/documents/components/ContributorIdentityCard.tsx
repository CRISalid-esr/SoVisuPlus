...
const ContributorIdentityCard = ({
  ...
  if (!isPerson(person)) {
    throw new Error('ContributorIdentityCard: agent is not a Person')
  }
        ...
        >
          <Stack spacing={2.2} alignItems='center' sx={{ width: '100%' }}>
            ...
            {contributor.affiliations.length > 0 && (
              ...
              >
                {contributor.affiliations.map((org, index, arr) => {
                  return (
                    <Box
                      key={org.uid}
                      component='span'
          ...
          {!person.external && (
            <Button
              variant={'contained'}
              sx={{ mt: 2 }}
              onClick={() => navigateToPerspective(person.slug!)}
            >
              <Trans>contributor_identity_card_see_profile_button</Trans>
            </Button>
          )}
        </Stack>
      </CardContent>
...
export default ContributorIdentityCard
