...
describe('PUT /api/documents/[uid]/titles', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('returns 400 if titles are missing or invalid', async () => {
    const request = {
      json: async () => ({ titles: [] }),
    } as Request
  ...
  it('calls modifyTitles and returns success', async () => {
    const request = {
      json: async () => ({
        titles: [
          { language: 'es', value: 'El nuevo titulo' },
    ...
    expect(modifyTitles).toHaveBeenCalledWith(
      document,
      [
        { language: 'es', value: 'El nuevo titulo' },
        { language: 'fr', value: 'Le nouveau titre' },
