...
jest.mock('@/lib/services/DocumentService', () => ({
  DocumentService: jest.fn().mockImplementation(() => ({
    ...
    modifyAbstracts: modifyAbstracts,
  })),
}))
...
describe('PUT /api/documents/[uid]/abstracts', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('returns 400 if abstracts are missing or invalid', async () => {
    const request = {
      json: async () => ({ abstracts: [] }),
    } as Request
  ...
  it('calls modifyAbstracts and returns success', async () => {
    const request = {
      json: async () => ({
        abstracts: [
          { language: 'es', value: 'El nuevo abstract' },
    ...
    expect(modifyAbstracts).toHaveBeenCalledWith(
      document,
      [
        { language: 'es', value: 'El nuevo abstract' },
        { language: 'fr', value: 'Le nouveau abstract' },
          ...
          [
            {
              action: PermissionAction.update,
              subject: PermissionSubject.Document,
              fields: ['titles', 'contributors', 'identifiers', 'documentType'],
            },
          ],
          [{ entityType: 'Person', entityUid: 'user-1234' }],
        ),
