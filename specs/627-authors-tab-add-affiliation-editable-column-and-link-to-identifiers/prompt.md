...
- All exit points from a dirty Authors tab are guarded: the tab bar, the sidebar navigation links, the sidebar account menu, the "back to publication list" button in the document header, and switching the viewing perspective from the sidebar search input (selecting another person/research-unit, or "back to my perspective"). Confirming "leave" replays the exact intended navigation; cancelling stays.
