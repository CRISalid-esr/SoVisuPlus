...
Profiles with **no identifier** — none of `idHal_s`, `orcidId_s` or `idrefId_s` — are dropped before the panel is built: their only purpose is to identify the contributor, and one carrying no identifier cannot. The filtering happens server-side (in `AureHalAPIClient.searchAuthorSuggestions`), so the '[N] matches' count reflects only the kept profiles, and a fully-filtered-out match set renders nothing (box not shown). This filtering applies **only** to these suggestions — the search-in-HAL autocomplete keeps showing identifier-less profiles (it has the explicit 'Add contributor' option).
