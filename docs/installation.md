...
| PERSPECTIVE_ROLES_FILTER                  | JSON array of contribution roles (LOC relator terms) linking a publication to the selected perspective: a document is listed for a person (or a structure's members) only if they contributed with at least one of these roles. Empty/unset disables the filter. Beware: `.env.sample` and `docker-bootstrap-app.sh` currently spell it `PERSPECTIVES_ROLES_FILTER` (plural), which the code does not read. | n       | server          | y                  | prod                                   | n                  |
| PUBLICATION_LIST_ROLES_FILTER             | JSON array of contribution roles (LOC relator terms) allowed to appear in the contributors column of the publication list: documents having at least one contributor with none of these roles are excluded from the list, so contributors holding only roles outside this list are never displayed. Empty/unset disables the filter.                                                                        | n       | server          | y                  | prod                                   | n                  |
