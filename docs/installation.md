...
The chat widget is displayed only when `CHAT_ENABLED` is not `false` **and** `CRISALID_AGENTS_API_URL` is set **and** a config file resolves. Set `CHAT_ENABLED=false` to turn the chat off explicitly (the widget is hidden and `/api/chat` returns 404).
