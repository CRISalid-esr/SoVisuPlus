...
| NEXT_PUBLIC_CRISALID_AGENTS_API_URL       | Base URL of the Crisalid Agents API backing the AI chat assistant (e.g. `http://crisalid-agents-chat-api:9100`); the app appends the `/chat` endpoint path. The chat widget is only displayed when this variable is set and non-empty.                | n       | client          | y                  | prod                                   | n                  |
