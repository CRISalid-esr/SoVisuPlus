;(function (window) {
  window.wordstream = function (svg, data, config) {
    d3.wordstreamLayout = function () {
      ...
      function buildBoxes(data) {
        ...
        topics.forEach((topic) => {
          ...
          dataPerTopic.push({
            x: 0,
            [topic]: frequencyScale(totalFrequencies[0][topic]),
          })
          totalFrequencies.forEach((frq, i) => {
            dataPerTopic.push({
              x: i * boxWidth + (boxWidth >> 1),
              [topic]: frequencyScale(frq[topic]),
            })
          })
          ...
          dataPerTopic.push({
            x: size[0],
            [topic]: frequencyScale(
              totalFrequencies[totalFrequencies.length - 1][topic],
            ),
          }) //TODO:
        ...
        let stack = d3.stack().keys(categories).offset(d3.stackOffsetWiggle)
      ...
      function place(word, board) {
        let bw = board.width,
          ...
          startX = ~~(
            board.boxX +
            ((board.boxWidth * (Math.random() + 0.5)) >> 1)
          ),
          startY = ~~(
            board.boxY +
      ...
      function buildSvg(boxes, topic) {
        ;((streamPath1 = Array()), (streamPath2 = Array()))
        let width = size[0],
          height = size[1]
        let svg = d3
      ...
      function angle(boxes) {
        ...
        for (let i = 0; i < d3.keys(innerBoxes).length; i++) {
          // i: index cua topic
          let key = d3.keys(innerBoxes)[i]

          for (let j = 0; j < innerBoxes[key].length - 1; j++) {
            // j: index cua doi tuong
      ...
      wordstream.frequencyScale = function (_) {
        return arguments.length
          ? ((frequencyScale = _), wordstream)
          : frequencyScale
      }
      wordstream.spiral = function (_) {
        return arguments.length
          ? ((spiral = spirals[_] || _), wordstream)
          : spiral
      }
    ...
    ;(function (data) {
      ...
      let height =
        globalHeight -
        (+margins.top + margins.bottom + axisPadding + legendHeight)
      ...
      axisGroup.attr(
        ...
        'translate(' +
          margins.left +
          ',' +
          (height + margins.top + axisPadding + legendHeight) +
          ')',
      )
      ...
      let xGridlineScale = d3
        .scaleBand()
        .domain(d3.range(0, dates.length + 1))
        .rangeRound([0, width + width / boxes.data.length])
      let xGridlinesAxis = d3.axisBottom(xGridlineScale)

      xGridlinesGroup.attr(
        'transform',
        'translate(' +
          (margins.left - width / boxes.data.length / 2) +
          ',' +
          (height + margins.top + axisPadding + legendHeight + margins.bottom) +
          ')',
      )

      let gridlineNodes = xGridlinesGroup.call(
        xGridlinesAxis
      ...
      opacity = d3.scaleLog().domain([minFreq, maxFreq]).range([0.4, 1])
      ...
      mainGroup.selectAll('.textData').on('click', function () {
        ...
        allTexts[0].forEach((t) => {
          ...
          d3.select(clonedParentNode)
            ...
            .style(
              'text-shadow',
              '-0.5px 0.5px 0 #fff, -0.5px -0.5px 0 #fff, 0.5px -0.5px 0 #fff, -0.5px 0.5px 0 #fff, 0.5px 0.5px 0 #fff',
            )
        })
      ...
      categories.forEach((topic) => {
        d3.select("path[topic='" + topic + "']").on('click', function () {
          ...
          document
            .querySelectorAll("g[cloned='true'][topic='" + topic + "']")
            .forEach((node) => {
              node.parentNode.removeChild(node)
            })
          //Remove the added path for it
          document
            .querySelectorAll("path[wordstream='true'][topic='" + topic + "']")
            .forEach((node) => {
              node.parentNode.removeChild(node)
            })
        })
      ...
      legendGroup.attr(
        ...
        'translate(' +
          margins.left +
          ',' +
          (height + margins.top + legendOffset) +
          ')',
      )
      let legendNodes = legendGroup
        .selectAll('g')
        .data(boxes.topics)
        .enter()
        .append('g')
